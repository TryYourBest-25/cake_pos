import { Controller, Get, Post, Body, Patch, Param, Delete, ParseIntPipe, HttpCode, HttpStatus, Query, BadRequestException, Req, Res, UseGuards } from '@nestjs/common';
...
export class PaymentController {
  ...
  @ApiOperation({ summary: 'Create a new payment record - Chỉ MANAGER và STAFF' })
  @ApiBody({ type: CreatePaymentDto })
  @ApiResponse({ status: 201, description: 'The payment has been successfully created.', type: CreatePaymentDto })
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(ROLES.MANAGER, ROLES.STAFF)
  @ApiOperation({ summary: 'Get a specific payment by ID - Chỉ MANAGER và STAFF' })
  @ApiParam({ name: 'id', description: 'Payment ID', type: Number })
  @ApiResponse({ status: 200, description: 'The payment details', type: CreatePaymentDto })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions' })
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(ROLES.MANAGER, ROLES.STAFF)
  @ApiOperation({ summary: 'Update an existing payment - Chỉ MANAGER và STAFF' })
  @ApiParam({ name: 'id', description: 'Payment ID', type: Number })
  ...
  @ApiResponse({ status: 200, description: 'VNPay payment URL created successfully', schema: { type: 'object', properties: { paymentUrl: { type: 'string' } } } })
  @ApiResponse({ status: 400, description: 'Bad Request' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  async handleVNPayCallback(
    ...
    if (result.success) {
      ...
      res.redirect(`${process.env.POS_URL || 'http://localhost:3001'}/payment/success?orderId=${result.payment?.order_id}`);
    } else {
      // Redirect to failure page hoặc trả về error response
      res.redirect(`${process.env.POS_URL || 'http://localhost:3001'}/payment/failure?message=${encodeURIComponent(result.message)}`);
    }
  }
  ...
  async handleVNPayWebhook(
    ...
    try {
      const result = await this.paymentService.processVNPayCallback(webhookData);
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(ROLES.MANAGER)
  @ApiOperation({ summary: 'Test endpoint for payment controller - Chỉ MANAGER' })
