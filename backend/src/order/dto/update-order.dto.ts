...
import { IsOptional, IsEnum, IsInt, Min, IsString, MaxLength, ValidateNested, ArrayMinSize } from 'class-validator';
...
export class UpdateOrderDto extends PartialType(CreateOrderDto) {
  @ApiProperty({ description: 'ID của nhân viên cập nhật (nếu có thay đổi)', example: 2, required: false })
  @IsOptional()
  @IsInt()
  @Min(1)
  @Type(() => Number)
  ...
  @ApiProperty({ description: 'ID của khách hàng cập nhật (nếu có thay đổi)', example: 2, required: false })
  @IsOptional()
  @IsInt()
  @Min(1)
  @Type(() => Number)
  ...
  @ApiProperty({ description: 'Ghi chú tùy chỉnh cập nhật', example: 'Đã xác nhận, giao gấp', maxLength: 1000, required: false })
  @IsOptional()
  @IsString()
  @MaxLength(1000)
  ...
  @ApiProperty({ description: '(Thay thế) Danh sách sản phẩm trong đơn hàng', type: [CreateOrderProductDto], required: false })
  @IsOptional()
  ...
  @ApiProperty({ description: '(Thay thế) Danh sách mã giảm giá áp dụng', type: [CreateOrderDiscountDto], required: false })
  @IsOptional()
  @ValidateNested({ each: true })
