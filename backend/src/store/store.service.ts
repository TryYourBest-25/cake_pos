...
export class StoreService {
  ...
  async create(createStoreDto: CreateStoreDto): Promise<store> {
    const { name, email, phone, opening_time, closing_time, opening_date, ...rest } = createStoreDto;
  ...
  async update(id: number, updateStoreDto: UpdateStoreDto): Promise<store> {
    const { email, name, opening_date, opening_time, closing_time, ...rest } = updateStoreDto;
    ...
    if (opening_time) dataToUpdate.opening_time = new Date(`1970-01-01T${opening_time}`);
    if (closing_time) dataToUpdate.closing_time = new Date(`1970-01-01T${closing_time}`);
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2003') { // Foreign key constraint failed
          throw new ConflictException(`Cannot delete store with ID ${id} due to existing related records.`);
        }
      }
      throw error;
    }
